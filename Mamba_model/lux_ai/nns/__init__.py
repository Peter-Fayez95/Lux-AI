...
from .mamba_blocks import RSimbaBlock, ConvToMamba, MambaToConv 
...
def _create_model(
    ...
    elif flags.model_arch == "mamba_model":
        base_model = nn.Sequential(
            conv_embedding_input_layer,
            ConvToMamba(),
            *[RSimbaBlock(
                embed_dim = flags.hidden_dim,
                height = MAX_BOARD_SIZE[0],
                width = MAX_BOARD_SIZE[1],
            ) for _ in range(flags.n_blocks)],
            MambaToConv(flags.embedding_dim,flags.embedding_dim)
        )
