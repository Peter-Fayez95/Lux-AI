...
class TestClass:
    ...
    def stringToMap(width, height, map_string):
        ...
        '''
        cells = [[Cell(resource=Resource('uranium', int(map_string[x][y])), x=x, y=y) for y in range(height)]
    ...
    def test_one(self):
        ...
        mapp = Map([[Cell(resource=Resource('uranium', 0), x=x, y=y) for y in range(height)] for x in range(width)], width, height)
